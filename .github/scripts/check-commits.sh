...
# Check for required environment variables, which are passed from the workflow
if [ -z "$TYPES" ] || [ -z "$MAX_LENGTH" ] || [ -z "$BASE_SHA" ] || [ -z "$HEAD_SHA" ]; then
  echo "Error: Required environment variables (TYPES, MAX_LENGTH, BASE_SHA, HEAD_SHA) are not set."
  exit 1
fi
