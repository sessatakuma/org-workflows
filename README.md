...
This is a repository with our shared workflows, **refactored to use Composite Actions** for better modularity and cross-repo portability.

## Architecture

The workflows are organized into **4 Category-Based Composite Actions**:

```
.github/
├── actions/
│   ├── basic-checks/          - PR title, branch name, commit messages, conflicts
│   ├── config-checks/         - YAML, JSON, TOML file validation
│   ├── frontend-checks/       - Prettier and ESLint checks
│   └── python-checks/         - Ruff formatting, linting, and Mypy type checking
├── scripts/
│   └── go-check-outcome.sh   - (still used by Go checks)
└── workflows/
    ├── entrypoint.yml            - Main orchestrator
    ├── main.yml                 - Sample caller workflow
    ├── reusables-basic.yml     - Calls basic-checks action
    ├── reusables-config.yml    - Calls config-checks action
    ├── reusables-frontend.yml   - Calls frontend-checks action
    ├── reusables-python.yml     - Calls python-checks action
    └── reusables-go.yml          - (unchanged)
```

### Why Composite Actions?

**Before**: Scripts were called directly via `run: .github/scripts/*.sh`, which failed when workflows were used from other repos (scripts weren't checked out).

**After**: Each category of checks is wrapped in a Composite Action using `${{ github.action_path }}` for reliable path resolution. This makes each check module independently reusable.
...
You can use any of these actions directly in your own workflows:
...
See [How to Add Workflows](HOW_TO_ADD_WORKFLOWS.md) for details on extending this system.
